...
| Aggregator fee obfuscation via positive slippage capture | Medium | If the aggregator captures fees as positive slippage rather than explicit transfer, `agg_fee_bps = 0` and the fee appears in `execution_quality_bps`. This is a known MVP limitation; document it per aggregator. |
