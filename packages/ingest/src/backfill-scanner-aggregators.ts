...
async function scanForAggregatorSwaps(): Promise<void> {
	...
	// Print CSV header
	console.log('\nblockNumber,transactionHash,logIndex,poolAddress,feeTier,aggregator,notionalUsd,amount0,amount1');

	// Print CSV rows
	for (const swap of results) {
		console.log(
			`${swap.blockNumber},${swap.transactionHash},${swap.logIndex},${swap.poolAddress},${swap.feeTier},${swap.aggregator},${swap.notionalUsd.toFixed(2)},${swap.amount0},${swap.amount1}`,
		);
	}
