...
async function samplePool005(): Promise<void> {
	...
	console.log(`Sampling USDC/WETH 0.05% pool for $${MIN_NOTIONAL.toLocaleString()}-$${MAX_NOTIONAL.toLocaleString()} swaps\n`);
	...
	for (const s of samples) {
		if (s.aggregator) {
			aggCount++;
			byAgg.set(s.aggregator, (byAgg.get(s.aggregator) ?? 0) + 1);
		}
	}
	...
	console.log(`\nThis pool is ${((aggCount / samples.length) * 100).toFixed(1)}% aggregator-routed — workable for dashboard!`);
}

samplePool005().catch((err) => {
