...
// A request the global ceiling refuses never reaches loadReceipt (see
// globalCeilingBlocksAnalysis.test.tsx), so it must not report a "receipt
// created" message either. The first call is the control: it's admitted and
// DOES notify, so the second call's flat count proves the ceiling, not an
// accident of the mock.
it('does not notify for a request throttled by the global hourly ceiling', async () => {
