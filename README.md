...
- **⚠️ `configs/*.json` paths are resolved from `import.meta.url`, which webpack bakes as an absolute build-time path.** Next bundles `@fabric-tca/core` from source, so the shipped bundle contains the literal path of whatever machine ran `next build`. This works today only because Nixpacks builds *inside* the deploy container, making that path the container's own. Introduce a Dockerfile, a CI-built artifact, or `output: 'standalone'` and all five registries (`routers`, `settlers`, `makers`, `reactors`, `entrypoints`) start silently falling back to empty — every one of them catches. `contractNames.ts` is the one that now searches from `process.cwd()` first; the others are unconverted.
...
- **⚠️ `/tx` is kept out of search indexes on purpose, as a cost control.** Every receipt URL is a fresh ~40-call analysis with no cache behind it, so an indexed receipt link is a standing invitation for a crawler to spend the global hourly budget and leave real visitors looking at the ceiling notice. `public/robots.txt` disallows `/tx/` and `/qa/` (this is the half that prevents the fetch, and so the spend), and `next.config.mjs` sets `X-Robots-Tag: noindex, nofollow` on the same paths for crawlers arriving from an external link. The index and `/methodology` stay crawlable. Receipts remain public and shareable — this limits discovery, not access.
