 ...
 *
 * For each corpus row this re-runs the SELECTION half of the bridged estimator
 ...
 *
 * Read-only. Much cheaper than a full decode (discovery + one balanceOf per
 * candidate, no route decomposition), but still ~62 x several RPC round trips.
 *
...
const rows = loadCorpus().filter((r) => r.block_number != null && r.input_token && r.output_token).slice(0, limit);

const out = [];
let n = 0;
for (const r of rows) {
	const refBlock = BigInt(r.block_number) - 1n;
	const inTok = norm(r.input_token);
	const outTok = norm(r.output_token);
	const rec = {
		id: r.id, tx: r.tx_hash, pair: `${r.input_symbol}->${r.output_symbol}`,
